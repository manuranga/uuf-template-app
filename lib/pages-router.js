        ...
        if (loginPageFullName) {
            ...
            if (loginPage) {
                ...
                if (loginPage.disabled) {
                    ...
                    var pageUri = loginPage.definition[constants.PAGE_DEFINITION_URI];
                    if (loginPage.collection) {
                        pageUri = '/' + loginPage.collection.substr(loginPage.collection.lastIndexOf(".") + 1) + pageUri;
                    }
                    return (appData.context + pageUri);
                }
            } else {
    ...
    function isPageProcessable(page, renderingContext, lookupTable, request, response) {
        ...
        if (user) {
            ...
            if (pagePermissions && Array.isArray(pagePermissions)) {
                ...
                var numberOfDivPermissions = pagePermissions.length;
                var userPermissionsMap = user.permissions;
                for (var i = 0; i < numberOfDivPermissions; i++) {
